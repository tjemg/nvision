/* LISTBOX1.CPP: Creating a simple list box  (rev 2)

   This sample code demonstrates one method of creating a simple list
   box which is inserted into a dialog box.

   Creating a list box requires an associated scroll bar and a list to
   display.  This example displays a list of the Turbo Vision classes.
...
/*=========================================== TApp =======================
   ListDialog - creates a dialog box containing a listbox
*/

void TApp::ListDialog() {

  // build a collection to pass to the listbox
  TStringCollection *theCollection = new TStringCollection(120, 10);

  for (int i=0; i<numStrings; i++)
    theCollection->insert(theList[i]);

  TDialog *pd = new TDialog(TRect(0, 0, 50, 20), "TV Classes");
  pd->options |= ofCentered;

  // create a scroll bar for the list box
  TScrollBar *listScroller = new TScrollBar(TRect(3, 17, 47, 18));
  ...
  // and a little clean up.  Note that we delete 'theCollection' rather
  // than destroy it.
