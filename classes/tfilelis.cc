...
#if defined(TVCompf_djgpp) // DJGPP
 #define Uses_fnmatch
 #include <dir.h>
#elif defined(TVComp_BCPP) || defined(TVCompf_Cygwin) || defined(TVOS_UNIX) && !defined(TVOSf_QNX4) // UNIX, Win32/BC++, Win32/Cygwin
 #define Uses_glob
#endif
...
void* TFileList::getKey( const char *s ) {
    ...
    if ((TGKey::getShiftState() & (kbLeftShiftDown | kbRightShiftDown)) || *s=='.')
        sR.attr=FA_DIREC;
    else
        sR.attr=0;
    strcpy( sR.name, s );
...
#else
#if defined(TVOS_Win32) && !defined(TVComp_BCPP) && !defined(TVCompf_Cygwin) // MingW
...
#else // Linux, BC++/Win32 and CygWin
...
static int ExcludeSpecialName(const char *name) {
    int len=strlen(name);
...
void TFileList::readDirectory( const char *aWildCard ) {
    ...
    if (dir) {
        while ((de = readdir(dir))) {
            ...
            char *name=de->d_name;
            if (!(ff.ff_attrib & FA_DIREC) && fnmatch(slash,name,0))
                continue;
            if (strcmp(name,".")==0)
                continue;
            if (removeParent && strcmp(name,"..")==0)
                continue;
            // SET: Special exclusions:
            if (ExcludeSpecialName(name))
                continue;
            p = new DirSearchRec;
            strcpy(p->name,name);
            p->attr = ff.ff_attrib;
...
#else
#if defined(TVOS_Win32) && !defined(TVComp_BCPP) && !defined(TVCompf_Cygwin) // Win32+MingW
...
void TFileList::readDirectory( const char *aWildCard ) {
    ...
    AllocLocalStr(wildcard,strlen(aWildCard)+1);
    strcpy(wildcard,aWildCard);
    slash = strrchr(wildcard,DIRSEPARATOR);
    ...
    if (dir!=-1) {
        do {
            ...
            if (strcmp(de.name,".")==0)
                continue;
            if (removeParent && strcmp(de.name,"..")==0)
                continue;
            if (ExcludeSpecialName(de.name))
                continue;
            p = new DirSearchRec;
            p->readFf_blk( de );
            fileList->insert(p);
...
void TFileList::readDirectory( const char *aWildCard ) {
    ...
    fileList = new TFileCollection( 5, 5 );
...
void TFileList::readDirectory( const char *aWildCard ) {
  ...
  fileList = new TFileCollection( 10, 10 );
  ...
  if ((dp = opendir(path)) != NULL) {
      while ((de = readdir(dp)) != NULL) {
          ...
          // is it a directory ?
          sprintf(path, "%s%s", dir, de->d_name);
          if (stat(path, &s) == 0 && S_ISDIR(s.st_mode) && !ExcludeSpecialName(de->d_name)) {
              p = new DirSearchRec;
              p->readFf_blk(de->d_name, s);
              fileList->insert( p );
          }
