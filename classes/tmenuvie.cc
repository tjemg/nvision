...
Boolean TMenuView::keyToItem(TEvent &event) {
    if (findItem(TGKey::GetAltChar(event.keyDown.keyCode, event.keyDown.charScan.charCode))) {
        putEvent(event);
        do_a_select(event);
        return True;
    }
        ...
        if ( p->name != 0 )
            if ( p->command == 0 ) {
                ...
                if ( (T = findHotKey( p->subMenu->items, keyCode )) != 0 )
                    return T;
            } else if ( !p->disabled && p->keyCode != kbNoKey && p->keyCode == keyCode)
                return p;
        p =  p->next;
    }
    return 0;
...
void TMenuView::writeMenu( opstream& os, TMenu *menu ) {
    ...
    for( TMenuItem *item = menu->items; item != 0; item = item->next ) {
        ...
        os << item->command << (short)(item->disabled) << item->keyCode << item->helpCtx;
        if ( item->name != 0 ) {
            if ( item->command == 0 )
