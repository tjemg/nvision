...
ushort doEditDialogDefault( int dialog, va_list arg ) {
    ...
    switch( dialog ) {
        case edOutOfMemory:
            return messageBox( __("Not enough memory for this operation."), mfError | mfOKButton );

        case edReadError: {
                              s=va_arg( arg, _charPtr );
                              va_end(arg);
                              return messageBox( mfError | mfOKButton, __("Error reading file %s."), s );
                          }

        case edWriteError: {
                               s=va_arg( arg, _charPtr );
                               va_end( arg );
                               return messageBox( mfError | mfOKButton, __("Error writing file %s."), s );
                           }

        case edCreateError: {
                                s=va_arg( arg, _charPtr );
                                va_end( arg );
                                return messageBox( mfError | mfOKButton, __("Error creating file %s."), s );
                            }
        ...
        case edSaveModify:
                            {
                                s=va_arg( arg, _charPtr );
                                va_end( arg );
                                return messageBox( mfInformation | mfYesNoCancel, __("%s has been modified. Save?"), s );
                            }

        case edSaveUntitled:
                            return messageBox( __("Save untitled file?"), mfInformation | mfYesNoCancel );

        case edSaveAs: {
                           return execDialog( new TFileDialog( "*", __("Save file as"), __("~N~ame"), fdOKButton, 101 ), va_arg( arg, _charPtr ) );
                       }
        ...
        case edSearchFailed:
                     return messageBox( __("Search string not found."), mfError | mfOKButton );
        ...
        case edReplacePrompt: {
                                  ...
                                  va_end( arg );
                                  return messageBoxRect( r, __("Replace this occurence?"), mfYesNoCancel | mfInformation );
                              }
        default:
                              return cmCancel;
    }
