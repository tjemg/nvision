...
void TEditor::handleEvent( TEvent& event ) {
    ...
    switch ( event.what ) {
        ...
        case evKeyDown:
            if ( event.keyDown.charScan.charCode == 9 || ( event.keyDown.charScan.charCode >= 32 && event.keyDown.charScan.charCode < 255 ) ) {
                lock();
...
void TEditor::setBufLen( uint32 length ) {
    ...
    limit.y  = countLines( buffer, bufLen ) + 1;
...
void TEditor::write( opstream& os ) {
    TView::write( os );
    os << hScrollBar << vScrollBar << indicator << bufSize << (short)canUndo;
}
